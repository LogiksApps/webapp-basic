...
function pgCreateNew() {
	refreshNeeded=true;
	$("#credsEditor").attr("src",_link("modules/userManager/"+currentSRC+"/new"));
}
...
function pgTrash() {
	...
	lgksConfirm("Are you sure about deleting the following ("+q1.length+") "+currentSRC+"?<br><div class='paddedBox'>"+q0.join("<br>")+"</div>","",function(ans) {
		if(ans) {
			processAJAXPostQuery(_service("userManager","delete"),"&src="+currentSRC+"&q="+q1.join(","),function(txt) {
				if(txt!="done") {
					lgksToast(txt);
				}
				pgRefresh();
...
function editRecord(tr,grid) {
	...
	$("#credsEditor").attr("src",_link("modules/userManager/"+currentSRC+"/edit/"+trHash));
}
function editPWD(tr,grid) {
	...
	console.log(_link("modules/userManager/"+currentSRC+"/pwd/"+trHash));
	$("#credsEditor").attr("src",_link("modules/userManager/"+currentSRC+"/pwd/"+trHash));
}
function infoRecord(tr,grid) {
	...
	$("#credsEditor").attr("src",_link("modules/userManager/"+currentSRC+"/userinfo/"+trHash));
}
// function cloneRecord(tr,grid) {
// 	trID=tr.find("td[data-key=id]").data("value");
// 	trHash=tr.data('hash');

// 	$("#credsEditor").attr("src",_link("modules/userManager/"+currentSRC+"/clone/"+trHash));
// }
function viewUsers(tr,grid) {
	...
	$("#credsEditor").attr("src",_link("modules/userManager/"+currentSRC+"/listusers/"+trHash));
}
