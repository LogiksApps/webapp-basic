...
$(function() {
	$("#roleModel").delegate("input[name=checkAll]","change",function(e) {
			...
			$(this).closest(".list-group.list-group-activity").find("input[name=roleCheckbox]").each(function() {
				...
				processAJAXPostQuery(_service("userRoles","save"),roleHash+"="+uniStatus,function(ans) {
					try {
						json=$.parseJSON(ans);
  ...
  $("#checkAllPermissions").change(function() {
	...
	$("#roleModel,#permissionList").delegate("input[name=roleCheckbox]","change",function(e) {
		...
		processAJAXPostQuery(_service("userRoles","save"),roleHash+"="+status,function(ans) {
			try {
				json=$.parseJSON(ans);
				if(json.Data=="success") {
					lgksToast("Updated");
...
function loadModuleList() {
	...
	processAJAXQuery(_service("userRoles","list-modules"),function(data) {
		html=[];
...
function loadPrivilegeList() {
  $("#componentTree>ul").html("<div class='ajaxloading ajaxloading5'></div>");
  processAJAXQuery(_service("userRoles","list-privileges"),function(data) {
        html=[];
...
function loadRoleModel(roleID) {
  ...
  processAJAXPostQuery(_service("userRoles","list-roles"),"&roleid="+roleID,function(data) {
...
function loadRoleForModule(moduleSlug) {
  ...
  processAJAXPostQuery(_service("userRoles","list-roles-modules"),"&module="+moduleSlug,function(data) {
...
function downloadRoleCSV() {
    window.open(_service("userRoles","downloadcsv"));
}

function uploadRoleCSV() {
