...
if(!isset($_COOKIE['USERROLES_UI'])) $uiType="by_roles";
else $uiType=$_COOKIE['USERROLES_UI'];
...
echo "<script>FORSITE='".SITENAME."';uiType='".$uiType."';</script>";
echo _js(["userRoles"]);
?>
<script>
...
function loadRoleModuleList(roleID) {
  ...
  processAJAXPostQuery(_service("userRoles","list-modules-for-role"),"&roleid="+roleID,function(data) {
          $("#roleModel").html("");
...
function loadActivtiesForRoleModule(roleid, module) {
  ...
  processAJAXPostQuery(_service("userRoles","list-activity-role-module"),"&roleid="+roleid+"&module="+module,function(data) {
          $("#activityList").html("");
...
function loadPermissionforActivityRoleModule(roleid, module,activity) {
  ...
  processAJAXPostQuery(_service("userRoles","list-permissions-activity-role-module"),"&roleid="+roleid+"&module="+module+"&activity="+activity,function(data) {
          $("#permissionList").html("");
...
function loadOLDUI() {
  $.cookie("USERROLES-UI","ui1");
  window.location = _link("modules/userRoles")+"&ui=ui1";
}
